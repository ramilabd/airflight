...
def get_flights_sorted_price(flights, reverse=False):
    return sorted(flights, key=lambda flight: float(flight['Price']['TicketPrice']), reverse=reverse)
...
def get_flights_sorted_direction(source, destination):
    ...
    for flight in all_flight:
        if flight['flight_1']['Source'] == source and 'flight_2' in flight:
            if flight['flight_2']['Destination'] == destination:
                sorted_flights.append(flight)
        elif flight['flight_1']['Destination'] == destination:
                sorted_flights.append(flight)
